...
func New(db *dynamodb.DynamoDB, sagaTable string) *SagaStore {
    return &SagaStore{
        db:    db,
        table: sagaTable,
    }
...
func (s *SagaStore) Get(ctx context.Context, id string, state *saga.State) errors.Error {
    res, err := s.db.GetItemWithContext(ctx, &dynamodb.GetItemInput{
        TableName:      &s.table,
        ConsistentRead: strongRead,
        Key: map[string]*dynamodb.AttributeValue{
            hashKeyK:  {S: ptr.String(hkVal)},
            rangeKeyK: {S: ptr.String(id)},
        },
    })

    if err != nil {
        return appErr.ErrUnableGetState.WithCaller().WithCause(err).WithInput(id)
    }

    if res.Item == nil {
        return appErr.ErrStateNotFound.WithCaller().WithInput(id)
    }

    state.Clear()
    if err := dynamodbattribute.UnmarshalMap(res.Item, state); err != nil {
        return appErr.ErrUnableUnmarshal.WithCaller().WithCause(err).WithInput(res.Item)
    }
...
func (s *SagaStore) Save(ctx context.Context, state *saga.State) errors.Error {
    ...
    item["rk"] = &dynamodb.AttributeValue{S: ptr.String(state.ID)}
