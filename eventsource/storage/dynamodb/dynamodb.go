...
type EventSourceStorage struct {
	db               *dynamodb.DynamoDB
	eventsourceTable string
	snapshotTable    string
}

func New(db *dynamodb.DynamoDB, eventsourceTable, snapshotTable string) *EventSourceStorage {
	return &EventSourceStorage{
		db:               db,
...
func (d *EventSourceStorage) Truncate() {
	output, err := d.db.Scan(&dynamodb.ScanInput{
		TableName: &d.eventsourceTable,
	})
	if err != nil {
		panic(err)
	}
...
func (d *EventSourceStorage) CreateSchema(enableStream bool) error {
...
func (d *EventSourceStorage) GetEvents(ctx context.Context, aggID string, seq int64) (event.Msgs, errors.Error) {
	keyCond := getCond
	...
	if err != nil {
		return nil, appErr.ErrUnableGetEventSource.WithCause(err).WithCaller().WithInput(errors.Input{
			"aggID": aggID,
			"seq":   seq,
		})
	}
...
func (d *EventSourceStorage) GetSnapshot(ctx context.Context, aggID string, version int) (*eventsource.Snapshot, errors.Error) {
	if version == 0 {
		return nil, nil
	}
	...
	if err != nil {
		return nil, appErr.ErrUnableGetEventSource.WithCause(err).WithCaller().WithInput(aggID)
	}
...
func (d *EventSourceStorage) Save(ctx context.Context, msgs event.Msgs, snapshot *eventsource.Snapshot) errors.Error {
...
func (d *EventSourceStorage) saveEvents(ctx context.Context, msgs event.Msgs) errors.Error {
	var err error
	...
	for i := 0; i < len(msgs); i++ {
		...
		if err != nil {
			return appErr.ErrUnableSaveEventSource.WithCause(err).WithCaller()
		}
		...
		_, err = d.db.PutItemWithContext(ctx, &dynamodb.PutItemInput{
			TableName:           ptr.String(d.eventsourceTable),
...
func (d *EventSourceStorage) saveSnapshot(ctx context.Context, snapshot *eventsource.Snapshot) errors.Error {
	if snapshot == nil {
		return nil
	}
	...
	if xerr != nil {
		return appErr.ErrUnableSaveEventSource.WithCause(xerr).WithCaller()
	}
