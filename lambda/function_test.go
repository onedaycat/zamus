...
type testWrapperHandler func(ctx context.Context, payload json.RawMessage) (interface{}, error)

func (h testWrapperHandler) Invoke(ctx context.Context, payload json.RawMessage) (interface{}, error) {
    return h(ctx, payload)
}

func TestInvoke(t *testing.T) {
    srv := &Function{handle: testWrapperHandler(
...
func TestPanic(t *testing.T) {
    ...
    assert.NoError(t, err)
    assert.Nil(t, response.Payload)
    assert.Equal(t, "CustomError2: Something bad happened!", response.Error.Message)
    assert.Equal(t, "CustomError2", response.Error.Type)
}
