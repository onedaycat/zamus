...
func (h *Handler) publish(ctx context.Context, streamName string) errors.Error {
    ...
    if err != nil {
        return appErr.ErrUnablePublishKinesis.WithCaller().WithCause(err)
    }

    if out.FailedRecordCount != nil && *out.FailedRecordCount > 0 {
        return appErr.ErrUnablePublishKinesis.WithCaller().WithCause(errors.New("One or more events published failed"))
    }
