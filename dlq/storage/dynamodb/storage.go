...
func New(db *dynamodb.DynamoDB, dlqTable string) *dlqDynamoDB {
    return &dlqDynamoDB{
        db:         db,
        dlqTable:   dlqTable,
...
func (d *dlqDynamoDB) Save(ctx context.Context, dlqMsg *dlq.DLQMsg) errors.Error {
    ...
    item["hk"] = &dynamodb.AttributeValue{S: ptr.String(string(dlqMsg.LambdaType))}
    item["rk"] = &dynamodb.AttributeValue{S: ptr.String(dlqMsg.ID)}
...
func (d *dlqDynamoDB) Get(ctx context.Context, lambdaType dlq.LambdaType, id string) (*dlq.DLQMsg, errors.Error) {
    res, err := d.db.GetItemWithContext(ctx, &dynamodb.GetItemInput{
        TableName:      &d.dlqTable,
        ConsistentRead: d.strongRead,
        Key: map[string]*dynamodb.AttributeValue{
            d.hashKeyK:  {S: ptr.String(string(lambdaType))},
            d.rangeKeyK: {S: ptr.String(id)},
        },
    })
